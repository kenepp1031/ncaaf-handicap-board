// Read-only public companion for the desktop app's app.js. This is a
// separate copy (app.js itself is never touched, so the live desktop tool
// keeps working unchanged) with every server-writing / live-polling code
// path removed or turned into a harmless notice, and every render*/matchup*
// function copied verbatim so the visual output stays pixel-identical to
// the desktop page.
//
// Data source: instead of polling /api/state, this reads the snapshot the
// host page assigned to window.__SNAPSHOT__ (produced by
// export_web_snapshot.py, shaped exactly like dashboard.py's /api/state
// response) once, synchronously, with no fetch/CORS involved.
let state={data:{events:[]},picks:[]},selected=null,activeTab=null,view='week',betsOnly=false,week=startWeek(new Date());const form=document.getElementById('pickform');const field=n=>form.elements.namedItem(n);const $=id=>document.getElementById(id);const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const signed=n=>n==null?'—':(n>=0?'+':'')+Number(n);function iso(d){return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`}function startWeek(d){let x=new Date(d.getFullYear(),d.getMonth(),d.getDate(),12);x.setDate(x.getDate()-(x.getDay()+6)%7);return x}function end(){if(activeTab)return iso(new Date(activeTab.end));let d=new Date(week);d.setDate(d.getDate()+6);return iso(d)}function inWeek(r){let e=r.event_id?(state.data.events||[]).find(e=>e.id===r.event_id):r;if(activeTab&&e&&e.week!=null)return e.season===state.data.season&&e.week===activeTab.number&&(e.season_type||2)===activeTab.type;return r.game_date>=iso(week)&&r.game_date<=end()}function notice(s){$('notice').textContent=s;$('notice').style.display='block'}
// No backend: this is a static, read-only snapshot published from the
// desktop app, so there is nothing to poll, refresh, save to, or recover a
// token from. initFromSnapshot() loads window.__SNAPSHOT__ once and renders.
function initFromSnapshot(){
 const snap=window.__SNAPSHOT__;
 if(!snap){notice('No board data was published with this page.');return}
 state=Object.assign({},snap,{data:snap.data||{events:[]},picks:snap.picks||[]});
 render();
}
function readOnlyNotice(){notice('Editing happens in the desktop app — this public page is read-only.')}
function refresh(){readOnlyNotice()}
function topUp(){/* background odds/weather top-up has no server to call here; no-op */}
function switchWeek(id){setTab(state.data.weeks.find(w=>w.id===id));clearForm();render();topUp()}
function currentTab(){let weeks=state.data.weeks||[],now=new Date();return weeks.find(w=>new Date(w.start)<=now&&now<=new Date(w.end))||weeks.find(w=>new Date(w.start)>now)||weeks.at(-1)}
...
function toggleFavorite(){readOnlyNotice()}
...
function quickPick(){readOnlyNotice()}
